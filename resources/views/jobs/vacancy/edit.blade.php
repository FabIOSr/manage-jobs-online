            ...
            <div class="col-md-10">
                ...
                <div class="row justify-content-start g-3">
                    <div class="col-md-8 ps-3">
                        <form action="{{ route('vacancies.update', $vacancy->id) }}" method="POST" autocomplete="off">
                            @csrf
                            <div class="row g-2">                                
                                <div class="col-md-9">
                                    <label for="vacancy" class="form-label mb-0">Tipo de vaga</label>
                                    <input name="vacancy" class="form-control form-control-sm" value="{{ old('vacancy', $vacancy->vacancy) }}" autocomplete="off" autofocus>
                                </div>
                                <div class="col-md-3">
                                    <label for="status" class="form-label mb-0">Status</label>
                                    <select name="status" class="form-select form-select-sm">
                                      <option value="choose" selected>...</option>
                                      <option value="ACTIVE" @if(old('status', $vacancy->status) == 'ACTIVE') selected @endif>ATIVO</option>
                                      <option value="INACTIVE" @if(old('status', $vacancy->status) == 'INACTIVE') selected @endif>INATIVO</option>
                                    </select>
                                  </div>
                                <div class="col-12">
